  ...
  NUI.component('inject-my-download', {
    ...
    methods: {
      getUnreadMsg: function () {
        tools.ajax('POST', this.url.getUnreadMsg, {}, function (data) {
          if (data) {
      ...
      loadMyDownloadData: function () {
        ...
        tools.ajax('POST', this.url.myDownload, this.form, function (data) {
          if (data) {
            self.table.loading = false
            self.table.data = data.list
            self.table.result_total = data.result_total
          }
        })
        this.getUnreadMsg()
      },
      ...
      rerun: function (id) {
        var self = this
        tools.ajax('POST', this.url.rerun, { id: id }, function (data) {
          if (data) {
            tools.showMsg('操作成功', 1)
            self.loadMyDownloadData()
          }
        })
      },
    ...
    template:
      '<ui-modal class="inject-my-download" ref="myDownload" size="auto" title="我的下载" @open="loadMyDownloadData">\
            <div class="wrap">\
                <!-- 条件筛选栏 -->\
                ...
                <section class="my-table">\
                    <my-simple-table :columns="columns" :data="table.data" :loading="table.loading" :custom-row="getCustomRow" :custom-cell="getCustomCell">\
                        <template slot="options" slot-scope="scope">\
                            ...
                            <ui-button v-if="scope.data.status == 0 || scope.data.status == 3" type="text" color="primary" @click="rerun(scope.data.id)">重新下载</ui-button>\
                        </template>\
                    </my-simple-table>\
                </section>\
